<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/assets/bootstrap/css/payment.css">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-GLhlTQ8iRABdZLl6O3oVMWSktQOp6b7In1Zl3/Jr59b6EGGoI1aFkw7cmDA6j6gD" crossorigin="anonymous">
<div class="payment">
    <div class="payment-tabs"><h2>Payment</h2></div>
    <div class="payment-customer">
        <h2>Ivan Yew</h2>
    </div>
    <form:form modelAttribute="payment" action="checkPayment" method="get" >
        <c:if test="${booking.id != null}">
            <input type="hidden" name="bookingID" value="${booking.id}">
        </c:if>
        <c:if test="${bookingID != null}">
            <input type="hidden" name="bookingID" value="${bookingID}">
        </c:if>
        <div class="payment-method">
        <h2>Detail booked: <a href="${pageContext.request.contextPath}/booking/searchBooking?searchKey=${sessionScope.external_ID}">Click here</a> </h2>
        <h2 style="color: red">Your Booking ID : ${sessionScope.external_ID}</h2>
        <h2>Choose your payment method</h2>
        <div class="pm-item">
            <form:radiobutton id="mpp" path="type_payment" value="Paypal" name="payment-method"/>
            <label class="pm-label" for="mpp" >
                <div class="pm-text" >
                    <h5>Paypal</h5>
                    <p>Safe payment online. Credit card needed. Paypal account is not necessary.</p>
                </div>
                <div class="pm-thumb"><img src="data:image/png;base64,"/></div>
            </label>
        </div>
        <div class="pm-item">
            <form:radiobutton id="mcc" path="type_payment" value="Credit Card" name="payment-method" checked="checked"/>
            <label class="pm-label" for="mcc">
                <div class="pm-text">
                    <h5>Credit Card</h5>
                    <p>Safe money transfer using your bank account. Safe payment online. Credit card needed. Visa, Maestro, Discover, American Express</p>
                </div>
                <div class="pm-thumb"><img src="data:image/png;base64,"/></div>
                <div class="pm-thumb"><img src="data:image/png;base64,"/></div>
            </label>
        </div>
    </div>
    <div class="payment-input">
        <div class="form-group success">
            <label for="ccnum">Credit Card Number</label>
            <form:input class="form-control is-valid" id="ccnum" type="text" path="cardNumber" name="ccnum" minlength="15" maxlength="15"/>
        </div>
        <div class="form-group half pr-3">
            <label for="ccmonth">Expiry Date</label>
            <div class="row mx-0">
                <div class="col-6 pl-0 pr-2">
                    <form:select path="expressMonth" class="form-control" id="ccmonth">
                        <option value="1">1</option>
                        <option value="2">2</option>
                        <option value="3">3</option>
                        <option value="4">4</option>
                        <option value="5">5</option>
                        <option value="6">6</option>
                        <option value="7">7</option>
                        <option value="8">8</option>
                        <option value="9">9</option>
                        <option value="10">10</option>
                        <option value="11">11</option>
                        <option value="12">12</option>
                    </form:select>
                </div>
                <div class="col-6 pl-2 pr-0">
                    <form:select path="expressYear" class="form-control" id="ccyear">
                        <option value="2018">2018</option>
                        <option value="2019">2019</option>
                        <option value="2020">2020</option>
                        <option value="2021">2021</option>
                        <option value="2022">2022</option>
                        <option value="2023">2023</option>
                        <option value="2024">2024</option>
                        <option value="2025">2025</option>
                    </form:select>
                </div>
            </div>
        </div>

        <div class="form-group">
            <label for="ccname">Name on Card</label>
            <form:input path="cardName" class="form-control" id="ccname" type="text" name="ccname" cssStyle="text-transform: uppercase" maxlength="30"/>
        </div>
        <div class="form-group">
            <label for="ccname"> Total price</label>
            $<form:input path="amount" class="form-control" readonly="true" id="ccname" type="text" value="${sessionScope.price}"/>
        </div>
    </div>
    <div class="payment-actions"><form:button class="finish" >Finish Order</form:button></div>
    </div>
    </form:form>